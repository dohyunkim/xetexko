...
\setsanshangulfont[Ligatures=TeX]{NanumBarunGothicOTF}
...
\newenvironment{plaintex}
  {\setbox0=\vbox\bgroup\hsize=\dimexpr\textwidth-12pt\relax\small}
  {\egroup \par\medskip
    ...
    \else
      ...
      \ifvoid0 \else\newpage\hbox{\grayvrule\kern10pt\box0}\fi
    \fi \par\medskip }
...
\begin{quote}\small
  ...
  \begin{itemize}\itemsep=0pt
    ...
    \item CJK 기호문자를 (한자가 아니라) 한글 글꼴에서 찾는 것이
      기본값이 되었다.
    \item \verb|interpunctskern| 글꼴 옵션을 없애고
      \verb|charraise| 및 \verb|quoteraise|를 추가했다.
    \item \verb|\fallbackhanjafont| 명령을 추가했다.
  \end{itemize}
\end{quote}
...
한편 \cs{fallbackhanjafont}를 정의해두면 \hemph{한자 글꼴에도 없는 글자}를
이 글꼴에서 찾아 식자한다. 간단히 \verb|\newfontfamily| 명령으로 정의한다.
...
0pt.
\item[charraise] 버전 2.2에서 새로 추가된 옵션.
  글자의 \hemph{수직 위치}를 조절한다. 다른 글꼴 관련 효과와
  충돌할 수 있어 주의를 요한다. \hemph{한자에 대해서만} 동작한다. 0pt.
\item[lowerperiod]
