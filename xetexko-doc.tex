...
\begin{quote}\small
  ...
  \begin{itemize}\itemsep=0pt
    ...
    \item 하이픈 문자를 \hparen{직전 글꼴이 아니라} 라틴 글꼴에서 찾는
      것이 기본값이 되었다.
    \item[v2.4]---
    \item 한자 변형 글리프 접근을 허용한다.
      제~\ref{sec:variationselectors} 절을 보라.
  \end{itemize}
\end{quote}
...
\label{sec:variationselectors}
트루타입의 cmap 테이블을 통해 변형 글리프로의 접근을 허용하는 폰트를
사용하는 경우 \verb|\prevfontcjksymbols| 또는 \verb|\hanjacjksymbols|
명령이 있어야 원하는 결과를 얻을 수 있다. 예컨대
\begin{verbatim}
  ...
  道\char"E0103
\end{verbatim}
\xetexko 는 Variation Selectors Supplement 영역을 CJK 상징기호로
인식하는데 이제 이들을 한글폰트에서 찾는 게 기본값이 되었기 때문이다.
