...
\date{Version 2.7\quad 2014/06/02}
\maketitle
...
\begin{verbatim}
  \usepackage{fontspec}
  \setmainfont{UnBatang}[Ligatures=TeX]
  \setsansfont{UnDotum}[Ligatures=TeX]
  \setmonofont{UnTaza}
...
\begin{verbatim}
  ...
  \setmainhangulfont{NanumMyeongjo}[Ligatures=TeX]
...
\end{verbatim}
main/sans hangul/hanja 폰트에는 따로 \verb|Ligatures=TeX|을 지시하지 않더라도
자동으로 주어진다.
...
\LaTeX 에서는 한글·한자 글꼴 옵션의 기본값을 지정할 수 있다.
\begin{verbatim}
